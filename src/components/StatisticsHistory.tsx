...
import { getStatisticsQuizHistory, getStatisticsQuizStats, clearStatisticsQuizHistory } from '../utils/statisticsUtils'
import { QuizHistoryEntry } from '../types/quiz'

const StatisticsHistory: React.FC = () => {
  ...
  const updateHistory = () => {
    setHistory(getStatisticsQuizHistory())
    setStats(getStatisticsQuizStats())
  }
  ...
  useEffect(() => {
    ...
    // Listen for custom event when statistics quiz is completed
    const handleStatisticsQuizCompleted = () => updateHistory()
    
    window.addEventListener('storage', handleStorageChange)
    window.addEventListener('statisticsQuizCompleted', handleStatisticsQuizCompleted)
    
    return () => {
      window.removeEventListener('storage', handleStorageChange)
      window.removeEventListener('statisticsQuizCompleted', handleStatisticsQuizCompleted)
    }
  }, [])

  const handleClearHistory = () => {
    if (window.confirm('Are you sure you want to clear all statistics quiz history?')) {
      clearStatisticsQuizHistory()
      setHistory([])
      setStats({
        totalQuizzes: 0,
        totalExams: 0,
  ...
  if (history.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-8 transition-colors duration-300">
        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-6 transition-colors duration-300">Statistics Quiz History</h3>
        <div className="text-center text-slate-500 dark:text-slate-400 py-12 transition-colors duration-300">
          ...
          <p className="text-lg font-medium text-slate-600 dark:text-slate-300 mb-2 transition-colors duration-300">No statistics quiz history yet</p>
          <p className="text-slate-500 dark:text-slate-400 transition-colors duration-300">Complete a statistics quiz to see your results here</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-8 transition-colors duration-300">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 transition-colors duration-300">Statistics Quiz History</h3>
        <button
              ...
              <div>
                ...
                <div className="text-slate-600 dark:text-slate-300 transition-colors duration-300">
                  {entry.mode === 'chapter' && entry.chapterInfo ? (
                    ...
                    <span className="text-orange-600 dark:text-orange-400 font-medium transition-colors duration-300">Advanced Statistical Concepts</span>
                  ) : null}
