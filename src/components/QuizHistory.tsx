...
import { getQuizHistory, getQuizStats, clearQuizHistory } from '../utils/quizUtils'
  ...
  if (history.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-8 transition-colors duration-300">
        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-6 transition-colors duration-300">Quiz History</h3>
        <div className="text-center text-slate-500 dark:text-slate-400 py-12 transition-colors duration-300">
          ...
          <p className="text-lg font-medium text-slate-600 dark:text-slate-300 mb-2 transition-colors duration-300">No quiz history yet</p>
          <p className="text-slate-500 dark:text-slate-400 transition-colors duration-300">Complete a quiz to see your results here</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-8 transition-colors duration-300">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 transition-colors duration-300">Quiz History</h3>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
        ...
        <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-2xl border border-green-200 dark:border-green-800 transition-colors duration-300">
          <div className="text-2xl font-bold text-green-600 dark:text-green-400 mb-1 transition-colors duration-300">{stats.totalExams}</div>
          <div className="text-sm text-green-700 dark:text-green-300 font-medium transition-colors duration-300">Exam Sims</div>
        </div>
          ...
          <div key={entry.id} className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border border-slate-200 dark:border-slate-600 hover:shadow-lg transition-all duration-300">
            ...
            <div className="text-right">
              <div className={`text-2xl font-bold mb-1 ${
                ...
                entry.percentage >= 60 ? 'text-yellow-600 dark:text-yellow-400' :
