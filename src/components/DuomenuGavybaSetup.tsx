...
import DuomenuGavybaHistory from './DuomenuGavybaHistory'
import { getDuomenuGavybaChapters } from '../utils/duomenuGavybaUtils'
import { duomenuGavybaQuestions, Question } from '../data/duomenuGavybaQuestions'
import { useTheme } from '../contexts/ThemeContext'

interface DuomenuGavybaSetupProps {
  onStartQuiz: (
...
const DuomenuGavybaSetup: React.FC<DuomenuGavybaSetupProps> = ({ onStartQuiz, totalQuestions }) => {
  const [instantFeedback, setInstantFeedback] = useState(false)
  const [showChapters, setShowChapters] = useState(false)
  const { isDarkMode } = useTheme()
  const chapters = useMemo(() => getDuomenuGavybaChapters(duomenuGavybaQuestions), [])
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-8 px-4 transition-colors duration-300">
      <div className="max-w-8xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl lg:rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-6 lg:p-8 mb-8 lg:mb-12 max-w-2xl mx-auto transition-colors duration-300">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-7 w-12 lg:h-8 lg:w-14 items-center rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl lg:rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 flex flex-col h-full">
              <div className="text-center flex flex-col h-full">
                ...
                <p className="text-slate-600 dark:text-slate-300 mb-3 lg:mb-4 leading-relaxed transition-colors duration-300 text-xs lg:text-sm">
                  Focus on specific data mining topics for targeted learning.
                </p>
          ...
          <div className="group">
            <div className="bg-white dark:bg-slate-800 rounded-2xl lg:rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 flex flex-col h-full">
              <div className="text-center flex flex-col h-full">
                ...
                <p className="text-slate-600 dark:text-slate-300 mb-3 lg:mb-4 leading-relaxed transition-colors duration-300 text-xs lg:text-sm">
                  Practice with all {totalQuestions} data mining questions.
                </p>
          ...
          <div className="group">
            <div className="bg-white dark:bg-slate-800 rounded-2xl lg:rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-4 lg:p-6 hover:shadow-2xl transition-all duration-300 flex flex-col h-full">
              <div className="text-center flex flex-col h-full">
                ...
                <p className="text-slate-600 dark:text-slate-300 mb-3 lg:mb-4 leading-relaxed transition-colors duration-300 text-xs lg:text-sm">
                  Take a quick test with 10 random data mining questions.
                </p>
        ...
        {/* Duomenu Gavyba Quiz History */}
        <DuomenuGavybaHistory />
      </div>
    </div>
  )
}

export default DuomenuGavybaSetup
